...
- ["] Quote `- ["]`
- [ ] High Priority #p1 
	- Uses the tag `#p1` to change color of checkbox
- [ ] Medium Priority #p2
- [ ] Low Priority #p3
...
> Author's phrase here. This uses `[!author]`.

> [!info|noicon]- Callout without Icon
> This uses `[!info|noicon]` to remove the icon. You can use this with any of my callouts as it primarily uses the `|noicon` for the title to remove the icon.

> [!bug|label] Unique label on left
> This places the label on the left. Get this with `[!bug|label]`. Just like the icon one above, this uses `|label` to get this look so it can be used with any of my callouts.

> [!m-callout] Minimal Callout
> This is a miminal callout. You can go into the CSS Snippet and change the border at the bottom. By default it has the musical notes but you can change it to anything and even create several with different bottoms if you know what you are doing OR submit an FR and I will make you one/several. You can get this look by using `[!m-callout]`.
> [!timeline|red] Title

> [!timeline|green] Title
> This is a timeline callout. You can change the color using `|color` right now it is `|green`. You can get this look by using `[!timeline|green]`.
